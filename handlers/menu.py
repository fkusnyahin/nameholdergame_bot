...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_character":
        ...
        await query.edit_message_text("⚔️ Используй /fight для боя")
    elif data == "menu_help":
...
async def show_character(query, context):
    """Показать персонажа с кнопками прокачки"""
...
async def handle_upgrade(query, context):
    ...
    # Создаём fake update для переиспользования существующих функций
    class FakeUpdate:
    ...
    # Обновляем экран персонажа
    await show_character(query, context)

async def exchange_particles(query, context):
    """Обмен частиц 20:1 вверх"""
    user_id = query.from_user.id
    data = load_player(user_id)
    
    # Пробуем обменять снизу вверх по всем тирам
    exchanged = False
    for tier in range(1, 4):  # 1->2, 2->3, 3->4
